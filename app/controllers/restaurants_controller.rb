class RestaurantsController < ApplicationController
  ...
  def show
  	...
  	unless ((params[:tab] == 'reviews') || (params[:tab] == 'about'))
  		@menu = Menu.new({restaurant_id: 'fatsos-last-stand-chicago-2', restaurant_lat: @restaurant.location.coordinate.latitude, restaurant_lon: @restaurant.location.coordinate.longitude, yelp_restaurant: {name: @restaurant.name, address: @restaurant.location.address[0], city: @restaurant.location.city}}).build_menu
  	end
